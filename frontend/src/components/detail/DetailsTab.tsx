...
import { updateDocument, classifyDocument, getDocument, rescanDocument, ApiError } from '@/lib/api';
...
interface Props {
  ...
  onDocUpdate: (doc: DocumentDetailType) => void;
}

export function DetailsTab({ doc, onDocUpdate }: Props) {
  ...
  const commitEdit = async (field: string) => {
    try {
      ...
      const updated = await updateDocument(doc.id, updates as Parameters<typeof updateDocument>[1]);
      onDocUpdate(updated);
    } catch {
      // silently fail
    } finally {
    ...
    <div className="flex flex-col">
      ...
      <div className="p-4 border-b border-zinc-800">
        <div className="flex items-center gap-2 mb-1">
          <span className="text-xs text-zinc-500">{t('detail.titleLabel')}</span>
        </div>
        <EditableCell
          field="title"
          displayValue={doc.title ?? '—'}
          rawValue={doc.title ?? ''}
        />
      </div>
      ...
      <div className="p-4 border-b border-zinc-800">
        <div className="grid grid-cols-2 gap-2 text-xs">
          ...
          <span className="text-zinc-500">{t('detail.yearLabel')}</span>
          <EditableCell
            field="year"
            ...
            {doc.classification_confidence !== null ? (
              <>
                ...
                {doc.classification_source && (
                  ...
                  >
                    {doc.classification_source === 'ai'
                      ? t('detail.classificationAI')
                      : t('detail.classificationUser')}
                  </span>
                )}
              </>
            ) : (
      ...
      {/* Rescan feedback (shown when status is error) */}
      {doc.processing_status === 'error' && (
        <div className="p-4 border-b border-zinc-800">
          ...
          {rescanFeedback && (
            <span className="ml-3 text-xs text-zinc-400">{rescanFeedback}</span>
          )}
        </div>
      )}
    </div>
